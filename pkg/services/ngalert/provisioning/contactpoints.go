...
func (ecp *ContactPointService) CreateContactPoint(ctx context.Context, orgID int64,
	...
	extractedSecrets, err := ExtractSecrets(contactPoint)
	if err != nil {
		return apimodels.EmbeddedContactPoint{}, err
	}
...
func (ecp *ContactPointService) UpdateContactPoint(ctx context.Context, orgID int64, contactPoint apimodels.EmbeddedContactPoint, provenance models.Provenance) error {
	...
	secretKeys, err := GetAllSecretKeys(contactPoint)
	if err != nil {
...
func GetAllSecretKeys(e apimodels.EmbeddedContactPoint) ([]string, error) {
...
func ExtractSecrets(e apimodels.EmbeddedContactPoint) (map[string]string, error) {
	secrets := map[string]string{}
	secretKeys, err := GetAllSecretKeys(e)
	if err != nil {
		return nil, err
	}
	for _, secretKey := range secretKeys {
